...
def calc_space_cube():
    for year in range(21, 42):
        obj = {}
        classify = np.load(f'./data/original/classify/classify-{year}.npy')
        value = np.load(f'./data/original/{year}.npy')
        ...
        np.save(f'./data/original/time-cube/space-cube-dict{year}.npy', obj)
        print(f'{year} was finished')


# 2.制作连接矩阵connect: [[]]
def calc_connect(year):
    last = np.load(f'./data/original/classify/st-id-{year}.npy')
    current = np.load(f'./data/original/classify/classify-{year + 1}.npy')
    img1 = np.load(f'./data/original/{year}.npy')
    img2 = np.load(f'./data/original/{year + 1}.npy')
    ...
    np.save(f'./data/original/time-cube/connect{year}.npy', connect)
    return connect
...
def build_first_cube(year):
    # year = 0
    first = np.load(f'./data/original/time-cube/space-cube-dict{year}.npy', allow_pickle=True).item()
    ...
    np.save(f'./data/original/time-cube/ST_cube_dict{year}.npy', obj)


def merge_cube(year):
    space_cube = np.load(f'./data/original/time-cube/space-cube-dict{year}.npy', allow_pickle=True).item()
    before = len(space_cube)
    print('before', before)
    st_cube = np.load(f'./data/original/time-cube/ST_cube_dict{year - 1}.npy', allow_pickle=True).item()
    connect = calc_connect(year - 1)
    print('connect', len(connect))
    classify = np.load('./data/original/classify/classify-0.npy')
    ...
    for key in space_cube:
        ...
        for cor in space_cube[key]['coordinate']:
            nowid[cor[0], cor[1]] = m
    np.save(f'./data/original/time-cube/ST_cube_dict{year}.npy', st_cube)
    np.save(f'./data/original/classify/st-id-{year}.npy', nowid)
...
if __name__ == "__main__":
    # ttt21 = np.load('./data/original/classify/classify-21.npy')
    # space_cube = np.load('./data/original/time-cube/space-cube-dict21.npy', allow_pickle=True).item()
    # 主要用于console的部分代码调试，主代码还是放在了main函数里面
    # calc_space_cube()
    # build_first_cube(0)
    main()
    # value = np.load('./data/original/test/value-0.npy')
    ST_cube_dict = np.load('./data/original/time-cube/ST_cube_dict20.npy', allow_pickle=True).item()
    count, num, yearsum = calc_res_index(ST_cube_dict)
    ...
    # for item in st_cube:
    #     for i in st_cube[item]:
    #         count += len(i['value'])
    # 首先来build first year的st—id ST-cube connect
    # merge_cube(1)
    # calc_connect
    # merge_cube(1)
    # for year in range(21):
    #     obj = np.load(f'./time-cube/space-cube-dict{year}.npy', allow_pickle=True).item()
    #     for key in obj:
    #         if len(obj[key]['value']) != len(obj[key]['coordinate']):
    #             print(year, key)
    # st_cube = np.load('./time-cube/ST_cube_dict20.npy', allow_pickle=True).item()
    # for key in st_cube:
    #     for i in st_cube[key]:
    #         if len(i['value']) != len(i['coordinate']):
    #             print(key)

    # year = 20
    # np.load('./time-cube/space-cube-dict1.npy')
    # space_cube = np.load(f'./time-cube/space-cube-dict{year}.npy', allow_pickle=True).item()
    # st_cube = np.load(f'./time-cube/ST_cube_dict{year}.npy', allow_pickle=True).item()
